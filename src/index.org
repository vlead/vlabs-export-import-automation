...
** Lab sepecification
    The JSON file with lab_name, lab_url and
    tar_file_location values will be the input to the above
    scripts. Update these values for exporting the importing
    different labs. This JSON file will be different for
    each lab.
